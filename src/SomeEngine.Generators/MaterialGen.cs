...
public sealed class MaterialGen : IIncrementalGenerator
{
    ...
    private static void EmitBind(MaterialInfo material, StringBuilder sb)
    {
        sb.AppendLine("    internal partial bool ToBindSet(global::SomeEngine.Render.RHI.ReflectedBinding binding, global::SomeEngine.Assets.AssetStore assets, out global::SomeEngine.Rhi.BindingResourceDesc resource)");
        sb.AppendLine("    {");
        sb.AppendLine("        resource = default;");
        if (material.Binds.Length != 0)
        {
            sb.AppendLine("        switch (binding.Name)");
            sb.AppendLine("        {");
            foreach (FieldInfo field in material.Binds.OrderBy(static field => field.Name, StringComparer.Ordinal))
            {
                sb.Append("            case \"").Append(Escape(field.Name)).AppendLine("\":");
                sb.AppendLine("            {");
                EmitBindCase(field, sb);
                sb.AppendLine("                break;");
                sb.AppendLine("            }");
            }
            sb.AppendLine("        }");
    ...
    private static void EmitBindCase(FieldInfo field, StringBuilder sb)
    {
        if (field.Type == "SomeEngine.Assets.Handle<SomeEngine.Render.Materials.Texture>")
        {
            sb.Append("                if (binding.Type == global::SomeEngine.Rhi.BindingType.TextureRead && ")
                .Append(field.Member)
                .AppendLine(".IsValid && assets.TryGet(")
                .Append("                    ")
                .Append(field.Member)
                .AppendLine(", out var texture) && texture is not null && texture.View.IsValid)");
            sb.AppendLine("                {");
            sb.AppendLine("                    resource = binding.Texture(texture.View);");
